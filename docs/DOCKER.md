...
```bash
VINE_WATCHER_IMAGE=gabrielepennacchia/vine-watcher:0.6.2 docker compose up -d watcher
```
...
The Docker workflow builds images on pushes and pull requests. It publishes to Docker Hub only when a semver tag such as `v0.6.2` is pushed.
